...
const envSchema = z.object({
  ...
  bunIdleTimeoutSeconds: z.coerce.number().int().positive().default(255),
  pipelineCallTimeoutMs: z.coerce.number().int().positive().default(12_000),
  pipelineMaxSubQuestions: z.coerce.number().int().positive().max(5).default(3),
...
export const env = envSchema.parse({
  ...
  port: Bun.env.PORT,
  bunIdleTimeoutSeconds: Bun.env.BUN_IDLE_TIMEOUT_SECONDS,
  pipelineCallTimeoutMs: Bun.env.PIPELINE_CALL_TIMEOUT_MS,
  pipelineMaxSubQuestions: Bun.env.PIPELINE_MAX_SUBQUESTIONS,
